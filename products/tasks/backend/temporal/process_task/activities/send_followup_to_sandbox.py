...
def _is_denied_permission_stop(run_id: str, error: str | None) -> bool:
    ...
    routinely fail together on this same diagnostic. Claiming the denial therefore has to be
    one locked read-modify-write: read and write as separate statements and both racers see
    it unclaimed, both brake, and the steer this guard exists to protect is the message that
    gets dropped.
    """
    if not error or TURN_ENDED_WITHOUT_RESPONSE_ERROR not in error:
        return False
...
def _deliver_followup(input: SendFollowupToSandboxInput) -> str | None:
    ...
    elif result.retryable and input.message_id and not _is_denied_permission_stop(input.run_id, result.error):
