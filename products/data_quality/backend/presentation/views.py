...
class _QualityGatedViewSet(TeamAndOrgViewSetMixin):
    ...
    def _denied_checks(self, checks: list[DataQualityCheck], denied: set[str]) -> set[UUID]:
        ...
        return {
            ...
            or self._reads_denied_subject(check.check_type, check.config, denied)
        }

    def _denied_definitions(self, runs: QuerySet[DataQualityCheckRun], denied: set[str]) -> Q:
        ...
        for check_type, config in definitions:
            if config is not None and self._reads_denied_subject(check_type, config, denied):
...
class DataQualityRunViewSet(
    ...
    def filter_queryset(self, queryset: QuerySet[DataQualitySuiteRun]) -> QuerySet[DataQualitySuiteRun]:
        ...
        return queryset.exclude(id__in=runs.filter(self._denied_definitions(runs, denied)).values("suite_run_id"))
