...
def fetch_ranked_observations(team_id: int, scanner_ids: list[str], ordered_ids: list[str]) -> list[ReplayObservation]:
    """Hydrate ranked observation ids into rows, preserving rank order. The scanner-id restriction re-applies
    the caller's access scope, so an embedding row can't leak an observation the caller can't read."""
    observations = {
        str(obs.id): obs
