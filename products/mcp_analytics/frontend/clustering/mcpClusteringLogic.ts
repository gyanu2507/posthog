...
// Cross-boundary contract with the recompute endpoint, which rejects a run this way when the
// organization hasn't approved AI data processing.
...
export interface mcpClusteringLogicMeta {
    __keaTypeGenInternalSelectorTypes: {
        ...
        aiConsentRequired: (dataProcessingAccepted: boolean, consentRejectedByApi: boolean) => boolean
    }
}
...
export const mcpClusteringLogic = kea<mcpClusteringLogicType>([
    ...
    reducers({
        ...
        // Backstop for a stale `currentOrganization` — the API is the authority on consent.
        consentRejectedByApi: [
            false,
            {
                triggerRecompute: () => false,
                triggerRecomputeSuccess: () => false,
                triggerRecomputeFailure: (_, { errorObject }) => isAiConsentError(errorObject),
            },
        ],
    }),
    selectors({
        ...
        // Clustering can't work without consent, so block it up front rather than only reacting
        // to a failed run — a snapshot that errored for this reason says nothing actionable.
        aiConsentRequired: [
            (s) => [s.dataProcessingAccepted, s.consentRejectedByApi],
            (dataProcessingAccepted: boolean, consentRejectedByApi: boolean): boolean =>
                !dataProcessingAccepted || consentRejectedByApi,
        ],
    }),
    listeners(({ actions, values, cache }) => ({
        ...
        triggerRecomputeFailure: ({ error, errorObject }) => {
            if (isAiConsentError(errorObject)) {
                // The consent prompt already explains this and offers the fix. Refresh the org
                // too: reaching here means the cached copy disagreed with the API.
