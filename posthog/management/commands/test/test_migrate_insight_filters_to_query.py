...
class TestMigrateInsightFiltersToQuery(BaseTest):
    ...
    @parameterized.expand(
        [
            ("trend", TREND_FILTERS, "TrendsQuery"),
            (
                "funnel",
                {
                    "insight": "FUNNELS",
            ...
            ("no_insight_key", {"events": [{"id": "$pageview", "type": "events", "order": 0}]}, "TrendsQuery"),
        ]
    )
    def test_converts_legacy_filters(self, _name, filters, expected_source_kind):
